<?php
/**
 * Epidermis - Modern version of MonoBook with fresh look and many usability
 * improvements.
 *
...
/**
 * QuickTemplate subclass for Epidermis
 * @ingroup Skins
 */
class EpidermisTemplate extends BaseTemplate {
	...
	public function execute() {
		...
		if ( $this->config->get( 'EpidermisUseIconWatch' ) ) {
		...
		$params = [
			...
			'msg-jumptonavigation' => $this->getMsg( 'epidermis-jumptonavigation' )->text(),
			'msg-jumptosearch' => $this->getMsg( 'epidermis-jumptosearch' )->text(),
		...
		foreach ( $portals as $name => $content ) {
			...
			switch ( $name ) {
				...
				case 'TOOLBOX':
					...
					Hooks::run( 'EpidermisAfterToolbox' );
					break;
				case 'LANGUAGES':
	...
	protected function renderNavigation( array $elements ) {
		...
		foreach ( $elements as $name => $element ) {
			switch ( $element ) {
				case 'NAMESPACES':
					?>
					<div id="p-namespaces" role="navigation" class="epidermisTabs<?php
				...
				case 'VARIANTS':
					?>
					<div id="p-variants" role="navigation" class="epidermisMenu<?php
					...
					?>" aria-labelledby="p-variants-label">
						...
						?>
						<input type="checkbox" class="epidermisMenuCheckbox" aria-labelledby="p-variants-label" />
				...
				case 'VIEWS':
					?>
					<div id="p-views" role="navigation" class="epidermisTabs<?php
				...
				case 'ACTIONS':
					?>
					<div id="p-cactions" role="navigation" class="epidermisMenu<?php
					if ( count( $this->data['action_urls'] ) == 0 ) {
						echo ' emptyPortlet';
					}
					?>" aria-labelledby="p-cactions-label">
						<input type="checkbox" class="epidermisMenuCheckbox" aria-labelledby="p-cactions-label" />
						<h3 id="p-cactions-label"><span><?php
							$this->msg( 'epidermis-more-actions' )
						?></span></h3>
				...
				case 'SEARCH':
					...
					<div id="p-search" role="search">
						...
						<form action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
							<div<?php echo $this->config->get( 'EpidermisUseSimpleSearch' ) ? ' id="simpleSearch"' : '' ?>>
								<?php
	...
	public function makeLink( $key, $item, $options = [] ) {
		...
		if ( isset( $options['epidermis-wrap'] ) && $options['epidermis-wrap'] ) {
	...
	public function makeListItem( $key, $item, $options = [] ) {
		...
		if (
			$this->config->get( 'EpidermisUseIconWatch' )
		...
		if (
			isset( $options['epidermis-collapsible'] ) && $options['epidermis-collapsible'] ) {
